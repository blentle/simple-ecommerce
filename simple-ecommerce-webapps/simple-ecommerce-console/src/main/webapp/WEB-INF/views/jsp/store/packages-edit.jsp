...
<head>
    <title>${platform_title}-编辑套餐</title>
...
<form id="createForm" class="definewidth m20">
    <input id="packagesId" value="${packages.id}" type="hidden"/>
    <table class="table table-bordered table-hover definewidth m10">
                ...
                <select id="countryCode" name="countryCode" onchange="loadStoresByCountry('${ctx}',this.value,'firstStoreValue');" class="required">
                    ...
                    <c:if test="${not empty countryList}">
                        <c:forEach var="countryEntity" items="${countryList}">
                            <option value="${countryEntity.countryCode}" <c:if test="${countryEntity.countryCode eq packages.countryCode}"> selected="selected" </c:if>>${countryEntity.countryName}</option>
                        </c:forEach>
                    </c:if>
                </select>
            </td>
                ...
                <select id="storeId" name="storeId" class="required">
                    ...
                    <c:if test="${not empty storeList}">
                        <c:forEach var="entity" items="${storeList}">
                            <option value="${entity.id}" <c:if test="${entity.id eq packages.storeId}"> selected="selected" </c:if>>${entity.storeName}</option>
                        </c:forEach>
                    </c:if>
                </select>
            </td>
        ...
        <tr>
            ...
            <td><input id="packagesName" name="packagesName" type="text" class="required" value="${packages.packagesName}"/></td>
        </tr>
        <tr>
            <td class="tableleft">套餐价格</td>
            <td>
                <input id="price" name="price" type="text" class="required number" value="${packages.packagesPrice}"/>
            </td>
        </tr>
        <tr>
            ...
            <td>
                <select id="packagesType" name="packagesType" class="required">
                    ...
                    <c:forEach var="typeEntity" items="${typeList}">
                        <option value="${typeEntity.code}"<c:if test="${typeEntity.code eq packages.packagesType.code}"> selected="selected" </c:if>><fmt:message key="def.packages.type.${typeEntity.code}.name" bundle="${def}"/></option>
                    </c:forEach>
                </select>
            </td>
        </tr>
        <tr>
            ...
            <td>
                <input id="salesVolume" name="salesVolume" type="text" class="required digits" value="${packages.salesVolume}">
            </td>
        </tr>
        <tr>
            <td class="tableleft">热度初始值</td>
            <td>
                <input id="hotVolume"  name="hotVolume" type="text" class="required digits" value="${packages.hotVolume}">
            </td>
        </tr>
        <tr>
            <td class="tableleft">标签</td>
            <td>
                <input type="checkbox" name="packagesTags" <c:if test="${packages.packagesTags.isIncludeNew()}"> checked </c:if> value="1"/>新品&nbsp;&nbsp;
                <input type="checkbox" name="packagesTags" <c:if test="${packages.packagesTags.isIncludeRecommendation()}"> checked </c:if> value="2"/>推荐&nbsp;&nbsp;
                <input type="checkbox" name="packagesTags" <c:if test="${packages.packagesTags.isIncludeHot()}"> checked </c:if> value="4"/>热销&nbsp;&nbsp;
            </td>
        </tr>
        <tr>
            ...
            <td>
                ...
                <div id="imgshow">
                    <img src="${resourceServer}${packages.packagesImage.m}"/>
                </div>
                <input id="imgJson" type="hidden" value="${imageJson}">
            </td>
        </tr>
        <tr>
            <td class="tableleft">是否上架</td>
            <td>
                <input type="radio" name="packagesStatus" value="y" <c:if test="${packages.packagesStatus.code eq 'y'}"> checked</c:if>/>上架
                <input type="radio" name="packagesStatus" value="n" <c:if test="${packages.packagesStatus.code eq 'n'}"> checked</c:if>/>下架
            </td>
        </tr>
        <tr>
            <td class="tableleft">适用范围</td>
            <td>
                <input id="range" name="range" type="text" class="required" value="${packages.packagesRange}"/>
            </td>
        </tr>
        <tr>
            <td class="tableleft">简介</td>
            <td>
                <textarea id="introduction">${packages.introduction}</textarea>
            </td>
        </tr>
        <tr>
            <td class="tableleft">有效期</td>
            <td>
                <input id="validTime" name="validTime" type="text" value="${packages.validTime}" class="required digits"/>(<span class="emphasize">单位:天</span>)
            </td>
        </tr>
        <tr>
            <td class="tableleft">描述</td>
            <td>
                <textarea id="desc">${packages.packagesDesc}</textarea>
            </td>
        </tr>
        <tr>
       ...
       $("#sbtn").bind("click",function() {
           ...
           if(imgJson != null && imgJson !="") {
               ...
               if(validateForm("#createForm")) {
                   $.post("${ctx}/store/packages/edit",data,function(result){
